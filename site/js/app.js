...
// ── State ─────────────────────────────────────────────────
let activities   = [];   // raw from JSON
let trails       = [];   // mapped for rendering
...
async function init() {
  ...
  // Labels on top so trails render between base and labels
  L.tileLayer(
...
async function loadActivities() {
  ...
  try {
    ...
    document.getElementById('activity-list').innerHTML =
      '<li style="padding:1rem;color:var(--text-3);font-size:13px">No activities yet.</li>';
    return;
  }
